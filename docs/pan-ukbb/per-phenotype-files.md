...
## Overview[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#overview "Direct link to heading")

The data are released in 7,228 flat files, one for each phenotype, and a corresponding tabix index file for each. These files are available on Amazon AWS (for large-scale analysis, we recommend using the [Hail format](https://pan.ukbb.broadinstitute.org/docs/Hail-format) files on Google Cloud).

The files are named with respect to their `trait_type`, `phenocode`, and a combination of `pheno_sex`, `coding`, or `modifier`. To find a specific phenotype, we suggest looking in the [phenotype manifest (Google Sheets)](https://docs.google.com/spreadsheets/d/1AeeADtT0U1AukliiNyiVzVRdLYPkTbruQSk38DeutU8/edit#gid=903887429) (available for download on [Amazon](https://pan-ukb-us-east-1.s3.amazonaws.com/sumstats_release/phenotype_manifest.tsv.bgz)). Search for your phenotype(s) of interest and use the paths indicated to download the summary statistics. A description of fields in the manifest can be found [here](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#phenotype-manifest-file).

The [per-phenotype files](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#per-phenotype-files) are summary statistics files containing meta-analyzed and single-ancestry GWAS results. We especially highlight the `low_confidence` fields, which includes some (non-exhaustive) basic quality control filters (see below). These files each have 28,987,534 variants, but note that not all populations will have data for each variant.

Finally, the [variant manifest file](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#variant-manifest-file) includes information on each variant in the dataset and has the same number of rows as each per-phenotype file. We highlight the `high_quality` column which represents variants that are PASS variants in gnomAD and have consistent frequencies with each population in gnomAD (AFR, AMR, EAS, and EUR frequencies are within 2-fold or chi-squared p-value of the difference > 1e-6).

## Phenotype manifest file[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#phenotype-manifest-file "Direct link to heading")
...
### Phenotype ID fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#phenotype-id-fields "Direct link to heading")
...
`pheno_sex`: Indicating whether the phenotype was run for both sexes (`pheno_sex`\="both\_sexes") or in just females (`pheno_sex`\="females") or males (`pheno_sex`\="males"). In 0.1, this is only differentiated for phecodes.
...
`n_cases_full_cohort_both_sexes`: Number of cases (or individuals phenotyped for quantitative traits) across all ancestry groups, females and males combined. Should be similar to the sum of per-ancestry n\_cases for relevant ancestries, but may include ancestry outliers and samples that failed QC.
...
### Population-specific fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#population-specific-fields "Direct link to heading")
...
### File information[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#file-information "Direct link to heading")
...
## Heritability manifest file[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#heritability-manifest-file "Direct link to heading")
...
### Phenotype ID fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#phenotype-id-fields-1 "Direct link to heading")
...
Unlike the phenotype manifest, the heritability manifest provides a row for each ancestry group. Thus the _first 6 fields_ are guarenteed to be unique.
...
### Heritability methods[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#heritability-methods "Direct link to heading")
...
### Heritability estimates[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#heritability-estimates "Direct link to heading")
...
`heritability.estimates.*.h2_z`: Heritability Z-scores for test of h2\>0h^2 > 0h2\>0.

`heritability.estimates.*.intercept`: LDSC intercept. Only present for `ldsc` and `sldsc_25bin`

`heritability.estimates.*.ratio`: LDSC ratio, given by intercept−1meanχ2−1\\frac{intercept-1}{mean \\chi^2 -1}meanχ2−1intercept−1​ as a measure of the proportion of test statistic inflation not attributed to polygenicity. Only present for `ldsc` and `sldsc_25bin`.

### QC flags[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#qc-flags "Direct link to heading")
...
`heritability.qcflags.significant_z`: if the ancestry-trait pair shows h2h^2h2 z-score \>0\> 0\>0

`heritability.qcflags.in_bounds_h2`: if, for all ancestries for a given trait, observed-scale heritability estimates ∈(0,1)\\in (0,1)∈(0,1)

`heritability.qcflags.normal_lambda`: if, for all ancestries for a given trait, λGC\>0.9\\lambda\_{GC} > 0.9λGC​\>0.9

`heritability.qcflags.normal_ratio`: if, for the top three best powered ancestry groups (EUR, CSA, AFR), the S-LDSC ratio, given by intercept−1meanχ2−1\\frac{intercept-1}{mean \\chi^2 -1}meanχ2−1intercept−1​, <0.3< 0.3<0.3 or the ratio z-score <4< 4<4
...
## Per-phenotype files[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#per-phenotype-files "Direct link to heading")
...
### Variant fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#variant-fields "Direct link to heading")
...
### Meta-analysis fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#meta-analysis-fields "Direct link to heading")
...
### High quality meta-analysis fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#high-quality-meta-analysis-fields "Direct link to heading")
...
### Population-specific fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#population-specific-fields-1 "Direct link to heading")
...
## Variant manifest file[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#variant-manifest-file "Direct link to heading")
...
### Variant fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#variant-fields-1 "Direct link to heading")

As in [per-phenotype files](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#Per-phenotype-files).
...
### Population-specific fields[#](https://pan.ukbb.broadinstitute.org/docs/per-phenotype-files/index.html#population-specific-fields-2 "Direct link to heading")
