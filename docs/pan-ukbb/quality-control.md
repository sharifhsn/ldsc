...
## Phenotypes[#](https://pan.ukbb.broadinstitute.org/docs/qc#phenotypes "Direct link to heading")
...
## Sample QC[#](https://pan.ukbb.broadinstitute.org/docs/qc#sample-qc "Direct link to heading")

-   We removed participants flagged with `putative.sex.chromosome.aneuploidy == 1`

### Ancestry definitions[#](https://pan.ukbb.broadinstitute.org/docs/qc#ancestry-definitions "Direct link to heading")
...
### Relatedness[#](https://pan.ukbb.broadinstitute.org/docs/qc#relatedness "Direct link to heading")
...
## Variant QC[#](https://pan.ukbb.broadinstitute.org/docs/qc#variant-qc "Direct link to heading")
...
## GWAS model[#](https://pan.ukbb.broadinstitute.org/docs/qc#gwas-model "Direct link to heading")
...
### Covariates[#](https://pan.ukbb.broadinstitute.org/docs/qc#covariates "Direct link to heading")
...
### Quality control of summary statistics[#](https://pan.ukbb.broadinstitute.org/docs/qc#quality-control-of-summary-statistics "Direct link to heading")

We have recently developed a best practice set of quality control steps for analysis of summary statistics from a wide range of phenotypes using the following _sequential_ filters:
...
4.  `significant_z`: if the ancestry-trait pair shows h2h^2h2 z-score \>0\> 0\>0
5.  `in_bounds_h2`: if, for all ancestries for a given trait, observed-scale heritability estimates ∈(0,1)\\in (0,1)∈(0,1)
6.  `normal_lambda`: if, for the top three best powered ancestry groups (EUR, CSA, AFR), λGC\>0.9\\lambda\_{GC} > 0.9λGC​\>0.9
7.  `normal_ratio`: if, for the top three best powered ancestry groups (EUR, CSA, AFR), the S-LDSC ratio, given by intercept−1meanχ2−1\\frac{intercept-1}{mean \\chi^2 -1}meanχ2−1intercept−1​, <0.3< 0.3<0.3 or the ratio z-score <4< 4<4
...
## Heritability estimation[#](https://pan.ukbb.broadinstitute.org/docs/qc#heritability-estimation "Direct link to heading")
...
-   RHEmc produces heritability estimates much more significantly different from 0 than LDSC/S-LDSC in non-EUR groups (see [post](https://pan.ukbb.broadinstitute.org/blog/2022/04/11/h2-qc-updated-sumstats)), however _we caution that the impact of stratification has yet to be fully explored on these estimates_ (as PCs are included only as fixed effects) and these may have a differential impact on different ancestry groups. However, these estimates have shown promise in summary statistics QC (see **Quality control of summary statistics**).
