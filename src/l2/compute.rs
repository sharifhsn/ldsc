...
// ── CountSketch Gram matrix fast path ─────────────────────────────────────────
//
// Replaces the O(d × W × M) cross-window GEMM with O(d² × M) Gram matrix
// quadratic forms. Two passes: forward (BED decode + Gram accumulation) and
// backward (stored sketch vectors only, no BED I/O).
//
// Identity: Σ_k (s_j^T s_k)² = s_j^T G s_j  where G = Σ_k s_k s_k^T (d×d).
// Bias correction: l2[j] = a · s_j^T G s_j + b · W_j + 1.0  (diagonal)
//   where a = active_n_inv_sq_r2u_a, b = active_r2u_b.

/// Rank-1 update: `gram += col * col^T` (f32 col promoted to f64 for Gram).
#[inline]
fn gram_rank1_update(gram: &mut MatF, col: &[f32], d: usize) {
    for i in 0..d {
        let vi = col[i] as f64;
        let gram_row = gram
            .col_mut(i)
            ...
            .as_slice_mut();
        for j in 0..d {
            gram_row[j] += vi * col[j] as f64;
        }
    }
}

/// Rank-1 downdate: `gram -= col * col^T`.
#[inline]
fn gram_rank1_downdate(gram: &mut MatF, col: &[f32], d: usize) {
    for i in 0..d {
        let vi = col[i] as f64;
        let gram_row = gram
            .col_mut(i)
...
/// Quadratic form: `s^T G s` with f32 sketch vector promoted to f64.
#[inline]
fn gram_quadratic_form(gram: &MatF, col: &[f32], d: usize) -> f64 {
    let mut result = 0.0f64;
    for i in 0..d {
        let vi = col[i] as f64;
        let gram_row = gram.col(i).try_as_col_major().unwrap().as_slice();
        let mut dot = 0.0f64;
        for j in 0..d {
            dot += gram_row[j] * col[j] as f64;
        }
        result += vi * dot;
    }
    result
...
/// CountSketch Gram matrix path. Returns `(l2, maf_per_snp)`.
///
/// This is a drop-in replacement for the main chunk loop in `compute_ldscore_global`
/// when CountSketch is active. It reads BED data once (forward pass), stores all
/// d-dimensional sketch vectors, then runs a backward pass on the stored vectors.
#[allow(clippy::too_many_arguments)]
fn countsketch_gram_path(
    all_snps: &[BimRecord],
    bed_path: &str,
    n_indiv: usize,
    mode: WindowMode,
    ...
    let m = all_snps.len();
    let n_annot = 1usize; // CountSketch is scalar-only
    ...
    // Chunk-aligned block sizes for window eviction (matches compute_ldscore_global)
    ...
    // Compute effective block_right matching the old code's cross-chromosome bleeding.
    // The old code's GEMM loop pairs ALL within-chunk SNPs (B×B) and ALL window SNPs
    // with chunk SNPs (A×B) without checking chromosome boundaries. So when a chunk
    // spans chromosomes, cross-chromosome pairs are included.
    // block_right_eff[j] = rightmost SNP index that contributes to l2[j] in the old code.
    let block_right_eff: Vec<usize> = {
        let mut br = vec![0usize; m];
        // Within-chunk: each SNP sees all others in the same chunk via B×B
        for cs in (0..m).step_by(chunk_c) {
            let ce = (cs + chunk_c).min(m);
            for item in br.iter_mut().take(ce).skip(cs) {
                *item = (*item).max(ce - 1);
            }
        }
        // Cross-chunk: SNP j is in the window of chunk C (at cs_C) when
        // a_left_C ≤ j < cs_C. Then j sees all SNPs in [cs_C, ce_C).
        for cs in (0..m).step_by(chunk_c) {
            let ce = (cs + chunk_c).min(m);
            let mut b = block_sizes[cs];
            if b > cs {
                b = cs;
            }
            let a_left = cs - b;
            // All SNPs in [a_left, cs) see chunk [cs, ce)
            for item in br.iter_mut().take(cs).skip(a_left) {
                *item = (*item).max(ce - 1);
            }
        }
        br
    ...
    // All sketch vectors: d × m (f32), stored column-major
    let mut all_sketches = MatF32::zeros(d, m);

    // Gram matrices (f64 for accumulation stability)
    let mut gram_fwd = MatF::zeros(d, d);

    // Per-SNP accumulators
    let mut l2_fwd = vec![0.0f64; m];
    let mut l2_bwd = vec![0.0f64; m];

    // Scratch: b_mat for fused decode output
    let mut b_mat = MatF32::zeros(d, chunk_c);
    // Scratch: c×c for B×B
    let mut bb = MatF32::zeros(chunk_c, chunk_c);

    // Timing
    let mut t_bed_read = std::time::Duration::ZERO;
    let mut t_sketch = std::time::Duration::ZERO;
    let mut t_gram = std::time::Duration::ZERO;

    // BED reader setup
    let mut seq_bed = Bed::builder(bed_path).build().context("opening BED")?;
    let mmap_bed: Option<MmapBed> = if use_mmap {
        Some(MmapBed::open(bed_path).context("mmap BED")?)
    } else {
        None
    };
    let mut chunk_reader = ChunkReader::new(&seq_bed, iid_indices, chunk_c, true, f32::NAN)
        .context("creating chunk reader")?;
    // Sequential mode if all SNPs contiguous
    ...
    if all_bed_sequential && !use_mmap {
        chunk_reader
            .start_sequential(&mut seq_bed, all_snps[0].bed_idx)
            .context("starting sequential BED read")?;
    }

    // ── Forward pass: BED decode + sketch + Gram accumulation ──────────────────
    let mut fwd_left_ptr = 0usize; // next SNP to evict from gram_fwd

    for chunk_start in (0..m).step_by(chunk_c) {
        let chunk_end = (chunk_start + chunk_c).min(m);
        let c = chunk_end - chunk_start;

        // Evict SNPs that left the window (chunk-aligned, matches old code)
        let mut b_fwd = block_sizes[chunk_start];
        if b_fwd > chunk_start {
            b_fwd = chunk_start;
        }
        let a_left = chunk_start - b_fwd;
        let t0 = Instant::now();
        while fwd_left_ptr < a_left {
            let col = all_sketches
                .col(fwd_left_ptr)
                .try_as_col_major()
                .unwrap()
                .as_slice();
            gram_rank1_downdate(&mut gram_fwd, &col[..d], d);
            fwd_left_ptr += 1;
        }
        t_gram += t0.elapsed();

        // Read BED + fused CountSketch decode+normalize+scatter-add → b_mat
        let t0 = Instant::now();
        let cr_ref = &chunk_reader;
        let bps = cr_ref.bytes_per_snp_val();
        let start_bed_idx = all_snps[chunk_start].bed_idx;
        let chunk_is_contiguous = all_snps[chunk_end - 1].bed_idx == start_bed_idx + c - 1;
        ...
        let raw: std::borrow::Cow<'_, [u8]> = if let Some(ref mb) = mmap_bed {
        ...
        } else if chunk_is_contiguous && all_bed_sequential {
            chunk_reader
                .read_next_raw_only(&mut seq_bed, c)
                .with_context(|| format!("reading BED chunk [{chunk_start},{chunk_end})"))?;
            std::borrow::Cow::Owned(chunk_reader.raw_bytes()[..c * bps].to_vec())
        } else if chunk_is_contiguous {
            chunk_reader
                .read_contiguous_raw_only(&mut seq_bed, start_bed_idx, c)
                .with_context(|| format!("reading BED chunk [{chunk_start},{chunk_end})"))?;
            std::borrow::Cow::Owned(chunk_reader.raw_bytes()[..c * bps].to_vec())
        } else {
            // Non-contiguous fallback: decode full genotypes then project
            ...
            let raw_mat: MatF32 = if let Some(iids) = iid_indices {
                builder.iid_index(iids).read(&mut seq_bed)
            } else {
                builder.read(&mut seq_bed)
            }
            .with_context(|| format!("reading BED chunk [{chunk_start},{chunk_end})"))?;
            // Project via CountSketch (non-fused path)
            cs.project_f32(&raw_mat, n_indiv, c, n, &mut b_mat);
            // Store and skip the fused path below
            for j in 0..c {
                let src = b_mat.col(j).try_as_col_major().unwrap().as_slice();
                let dst = all_sketches
                    .col_mut(chunk_start + j)
                    .try_as_col_major_mut()
                    .unwrap()
                    .as_slice_mut();
                dst[..d].copy_from_slice(&src[..d]);
            }
            // Compute MAF from raw_mat
            for j in 0..c {
                let raw_col = raw_mat.col(j).try_as_col_major().unwrap().as_slice();
                let mut s = 0f64;
                let mut cnt = 0usize;
                for &v in &raw_col[..n_indiv] {
                    if !v.is_nan() {
                        s += v as f64;
                        cnt += 1;
                    }
                ...
                maf_per_snp[chunk_start + j] = mean / 2.0;
            }
            ...
            // Skip fused path — already projected
            let t0 = Instant::now();
            // Gram operations for this chunk (cross-chunk + B×B + rank-c update)
            gram_ops_forward_chunk(
                &mut gram_fwd,
                &all_sketches,
                &mut l2_fwd,
                fwd_left_ptr,
        ...
        // Fused CountSketch
        let t0 = Instant::now();
        let n_i = chunk_reader.n_iid();
        let ipos = chunk_reader.iid_positions_ref().to_vec();
        let ai = chunk_reader.all_iids_flag();
        ...
        // Gram operations
        let t0 = Instant::now();
        // Cross-chunk: l2_fwd[j] += s_j^T G s_j * a + W * b
        for j in 0..c {
            let snp_idx = chunk_start + j;
            let col = all_sketches
                .col(snp_idx)
                .try_as_col_major()
                .unwrap()
                .as_slice();
            // Number of past window SNPs = all in [block_left[snp_idx], chunk_start+j)
            // that have already been added to gram_fwd.
            // Cross-chunk portion only: SNPs from fwd_left_ptr to chunk_start.
            // Within-chunk (k < j) handled by B×B below.
            let w_cross = chunk_start.saturating_sub(fwd_left_ptr);
            if w_cross > 0 {
                let qf = gram_quadratic_form(&gram_fwd, &col[..d], d);
                l2_fwd[snp_idx] += qf * active_n_inv_sq_r2u_a + w_cross as f64 * active_r2u_b;
            }
        }

        // Within-chunk B×B (lower triangle → l2_fwd)
        {
            let b_slice = mat_slice_f32(b_mat.as_ref(), 0..d, 0..c);
            let mut bb_sl = mat_slice_mut_f32(bb.as_mut(), 0..c, 0..c);
            matmul_tn_to_f32(
                bb_sl.as_mut(),
                b_slice,
                b_slice,
            ...
            for j in 0..c {
                let mut sum = 0.0f64;
                for k in 0..j {
                    let val = bb[(k, j)] as f64;
                    sum += val * val * active_n_inv_sq_r2u_a + active_r2u_b;
                }
                l2_fwd[chunk_start + j] += sum;
            }
        }

        // Rank-c Gram update: gram_fwd += S_chunk @ S_chunk^T
        for j in 0..c {
            let col = all_sketches
                .col(chunk_start + j)
                .try_as_col_major()
                .unwrap()
                .as_slice();
            gram_rank1_update(&mut gram_fwd, &col[..d], d);
        }
        t_gram += t0.elapsed();
    }

    // ── Backward pass: stored sketch vectors only, no BED I/O ─────────────────
    let mut gram_bwd = MatF::zeros(d, d);
    let mut bwd_right_ptr = m; // next SNP to evict (exclusive, decrements)

    // Collect chunk starts for reverse iteration
    let chunk_starts: Vec<usize> = (0..m).step_by(chunk_c).collect();

    let t0_bwd = Instant::now();
    for &chunk_start in chunk_starts.iter().rev() {
        let chunk_end = (chunk_start + chunk_c).min(m);
        let c = chunk_end - chunk_start;

        // Evict SNPs past block_right boundary from gram_bwd
        let a_right = block_right_eff[chunk_end - 1]; // rightmost effective window
        while bwd_right_ptr > a_right + 1 {
            bwd_right_ptr -= 1;
            let col = all_sketches
                .col(bwd_right_ptr)
                .try_as_col_major()
                .unwrap()
                .as_slice();
            gram_rank1_downdate(&mut gram_bwd, &col[..d], d);
        }

        // Cross-chunk: l2_bwd[j] += s_j^T G_bwd s_j * a + W * b
        for j in 0..c {
            let snp_idx = chunk_start + j;
            let col = all_sketches
                .col(snp_idx)
                ...
                .unwrap()
                .as_slice();
            // Number of future window SNPs beyond the chunk
            let w_cross = bwd_right_ptr.saturating_sub(chunk_end);
            if w_cross > 0 {
                let qf = gram_quadratic_form(&gram_bwd, &col[..d], d);
                l2_bwd[snp_idx] += qf * active_n_inv_sq_r2u_a + w_cross as f64 * active_r2u_b;
            }
        }

        // Within-chunk B×B (upper triangle → l2_bwd)
        {
            // Recompute R = S_chunk^T @ S_chunk from stored sketch vectors
            // Copy chunk into contiguous scratch for GEMM
            let mut s_chunk = MatF32::zeros(d, c);
            for j in 0..c {
                let src = all_sketches
                    .col(chunk_start + j)
                    .try_as_col_major()
                    .unwrap()
                    .as_slice();
            ...
            let mut r_mat = MatF32::zeros(c, c);
            matmul_tn_to_f32(
                r_mat.as_mut(),
                s_chunk.as_ref(),
                s_chunk.as_ref(),
            ...
            for j in 0..c {
                let mut sum = 0.0f64;
                for k in (j + 1)..c {
                    let val = r_mat[(j, k)] as f64;
                    sum += val * val * active_n_inv_sq_r2u_a + active_r2u_b;
                }
                l2_bwd[chunk_start + j] += sum;
            }
        }

        // Rank-c Gram update: gram_bwd += S_chunk @ S_chunk^T
        for j in 0..c {
            let col = all_sketches
                .col(chunk_start + j)
                .try_as_col_major()
                .unwrap()
                .as_slice();
            gram_rank1_update(&mut gram_bwd, &col[..d], d);
        }
    }
    t_gram += Instant::now().duration_since(t0_bwd);

    // ── Combine: l2 = l2_fwd + l2_bwd + 1.0 (diagonal) ──────────────────────
    for j in 0..m {
        l2[(j, 0)] = l2_fwd[j] + l2_bwd[j] + 1.0;
    }

    if verbose_timing {
        eprintln!(
            "[perf] countsketch_gram: bed_read={:.3}s sketch={:.3}s gram={:.3}s",
...
/// Helper: apply Gram operations for a chunk during forward pass.
/// Used by the non-contiguous fallback path.
#[allow(clippy::too_many_arguments)]
fn gram_ops_forward_chunk(
    gram_fwd: &mut MatF,
    all_sketches: &MatF32,
    l2_fwd: &mut [f64],
    fwd_left_ptr: usize,
    ...
    // Cross-chunk quadratic forms (read gram_fwd before mutating it)
    let w_cross = chunk_start.saturating_sub(fwd_left_ptr);
    if w_cross > 0 {
        for j in 0..c {
            let snp_idx = chunk_start + j;
    ...
    // Within-chunk B×B (lower triangle)
    {
        let mut s_chunk = MatF32::zeros(d, c);
        for j in 0..c {
            let src = all_sketches
        ...
        let bb_sl = mat_slice_mut_f32(bb.as_mut(), 0..c, 0..c);
        matmul_tn_to_f32(
            bb_sl,
            s_chunk.as_ref(),
            s_chunk.as_ref(),
    ...
    for j in 0..c {
        let col = all_sketches
            ...
            .unwrap()
            .as_slice();
        gram_rank1_update(gram_fwd, &col[..d], d);
    }
}
...
pub(super) fn compute_ldscore_global(
    ...
    // ── CountSketch Gram matrix fast path ────────────────────────────────────
    // Dispatch to the Gram path when CountSketch is active, stochastic is off,
    // and --gram is enabled. Replaces O(d × W × M) cross-window GEMM with
    // O(d² × M) quadratic forms. Best when windows are large relative to d².
    if let Some(ref cs) = count_sketch
        && (stochastic.is_none() || stochastic == Some(0))
        && use_gram
    {
        let d = sketch_dim.unwrap();
        return countsketch_gram_path(
            all_snps,
            bed_path,
            n_indiv,
            mode,
