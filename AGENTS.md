...
- `src/` holds the Rust implementation. Entry points and subcommands are in `main.rs` and `cli.rs`; core pipelines live in modules like `munge.rs`, `l2/` (submodule directory), `regressions.rs`, `jackknife.rs`, and `irwls.rs`.
...
- If adding complex logic, include brief comments for non-obvious invariants (see `src/l2/compute.rs` and `src/regressions.rs`).
